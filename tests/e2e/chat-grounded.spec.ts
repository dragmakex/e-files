...
test("chat screen exposes accessible chat shell without the old intro copy", () => {
  const html = renderToStaticMarkup(createElement(ChatPage))

  expect(html).not.toContain("Grounded Chat")
  expect(html).not.toContain("Answers are grounded in indexed documents")
